...
const buildEditor = ({
	...
	const savePng = (projectId: string) => {
		...
		downloadFile(dataUrl, "png", projectId);
	...
	const saveJpg = (projectId: string) => {
		const options = generateSaveOptions();
		const currentTransform = canvas.viewportTransform
			? [...canvas.viewportTransform]
			: null;
		...
		canvas.setViewportTransform([1, 0, 0, 1, 0, 0]);
		const dataUrl = canvas.toDataURL(options);
		downloadFile(dataUrl, "jpg", projectId);
	...
	const saveSvg = (projectId: string) => {
		const options = generateSaveOptions();
		const currentTransform = canvas.viewportTransform
			? [...canvas.viewportTransform]
			: null;
		...
		canvas.setViewportTransform([1, 0, 0, 1, 0, 0]);
		const dataUrl = canvas.toDataURL(options);
		downloadFile(dataUrl, "svg", projectId);
	...
	const saveJson = async (projectId: string) => {
		...
		downloadFile(fileString, "json", projectId);
	};
	...
	const getWorkSpace = () => {
		...
		if (objects.length > 0) {
			return objects[0] as fabric.Rect;
		}
		return undefined;
	};
